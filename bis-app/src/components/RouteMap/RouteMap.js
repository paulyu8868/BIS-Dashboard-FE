...
export default function RouteMap({ routes }) {
    ...
    const clearMapObjects = React.useCallback(() => {
        mapObjectsRef.current.polylines.forEach(polyline => polyline.setMap(null));
        mapObjectsRef.current.markers.forEach(marker => marker.setMap(null));
        mapObjectsRef.current.infoWindows.forEach(infoWindow => infoWindow.close());
    ...
    const updateRoutes = React.useCallback(() => {
        ...
        const colors = [
            ...
            "#00FFFF", "#FFC0CB", "#808080", "#FFFF00", "#8B4513",
        ];
        ...
        routes.forEach((route, index) => {
            ...
            if (route.vertices && route.vertices.length > 0) {
                const path = route.vertices.map(vertex => 
