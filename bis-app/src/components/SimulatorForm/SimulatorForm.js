...
const SimulatorForm = ({ map }) => {
    ...
    const [selectedRouteName, setSelectedRouteName] = useState("노선을 선택하세요");
    ...
    useEffect(() => {
        ...
        fetch(`${process.env.REACT_APP_BACKEND_URL}/api/simulator/buses/${selectedRoute}`)
    ...
    const startSimulation = async () => {
        ...
        try {
            console.log("Starting simulation with route:", selectedRoute, "bus:", selectedBus);
            const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/api/simulator/start`, {
                method: "POST",
            ...
            const initialPosition = new window.kakao.maps.LatLng(firstVertex.ycord, firstVertex.xcord);
    ...
    const startLiveUpdate = () => {
        ...
        intervalRef.current = setInterval(async () => {
            try {
                ...
                const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/api/simulator/locations/${selectedRoute}`);
                const locations = await response.json();

                if (locations.length > 0) {
                    const { xCord, yCord } = locations[0];
